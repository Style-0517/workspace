...
import {
  ...
  EMA_PULLBACK_SIGNAL_TYPE,
  EMA_PULLBACK_TREND_STRATEGY_ID,
  detectEmaPullbackTrendContinuationSignal,
  getRequiredEmaPullbackCandles,
} from "./emaPullbackTrendStrategy.js";

export const DEFAULT_EMA_PULLBACK_SIGNAL_SEED_LIMIT = 60;
export const DEFAULT_EMA_PULLBACK_SIGNAL_ALERT_LIMIT = 50;
...
function createRecommendation() {
  return "manual-long-paper-trade";
}
...
function createAlert({
  ...
  const candleCloseTime =
    detection.confirmationCandle.closeTime ??
    detection.confirmationCandle.eventTime ??
    null;
  const id = [
    "ema-pullback",
    market.symbol,
  ...
  return {
    ...
    fastPeriod: detection.fastPeriod,
    slowPeriod: detection.slowPeriod,
    emaGapPercent: detection.emaGapPercent,
    closeStrength: detection.closeStrength,
    reclaimDistancePercent: detection.reclaimDistancePercent,
    confidence: detection.confidence,
    explanation: detection.explanation,
    confirmationCandle: { ...detection.confirmationCandle },
    pullbackCandle: { ...detection.previousCandle },
...
export function createEmaPullbackTrendSignalEngine({
  client,
  watchlistSize = DEFAULT_BINANCE_UNIVERSE_CONFIG.size,
  intervals = SIGNAL_INTERVALS,
  seedLimit = DEFAULT_EMA_PULLBACK_SIGNAL_SEED_LIMIT,
  maxCandles = seedLimit,
  alertLimit = DEFAULT_EMA_PULLBACK_SIGNAL_ALERT_LIMIT,
  strategyOptions = DEFAULT_EMA_PULLBACK_OPTIONS,
  ...
  const minimumSeedCandles =
    getRequiredEmaPullbackCandles(mergedStrategyOptions);
  const listeners = new Set();
  const emittedAlertIds = new Set();

  if (seedLimit < minimumSeedCandles) {
    throw new Error(
      `seedLimit must be at least ${minimumSeedCandles} candles for EMA pullback detection`,
    );
  }

  if (maxCandles < minimumSeedCandles) {
    throw new Error(
      `maxCandles must be at least ${minimumSeedCandles} candles for EMA pullback detection`,
    );
  }
  ...
  const evaluateSignal = (symbol, interval, eventTime) => {
    ...
    const detection = detectEmaPullbackTrendContinuationSignal(
      bucket.candles,
