...
import {
  ...
  BOX_RANGE_BREAKOUT_STRATEGY_ID,
  BOX_RANGE_BREAKOUT_SIGNAL_TYPE,
  DEFAULT_BOX_RANGE_BREAKOUT_OPTIONS,
  detectBoxRangeBreakoutSignal,
} from "./boxRangeBreakoutStrategy.js";

export const DEFAULT_SIGNAL_SEED_LIMIT = 60;
export const DEFAULT_SIGNAL_ALERT_LIMIT = 50;
...
function createAlert({
  ...
  const confirmedSignal = createConfirmedSignalAlert({
    ...
    direction: detection.direction === "bullish" ? "long" : "short",
  ...
  return {
    ...
    tradeBias: detection.direction === "bullish" ? "long" : "short",
    ...
    breakoutBoundary: detection.breakoutBoundary,
    boxHigh: detection.boxHigh,
    boxLow: detection.boxLow,
    rangePercent: detection.rangePercent,
...
export function createBoxRangeBreakoutSignalEngine({
  client,
  watchlistSize = DEFAULT_BINANCE_UNIVERSE_CONFIG.size,
  intervals = SIGNAL_INTERVALS,
  seedLimit = DEFAULT_SIGNAL_SEED_LIMIT,
  maxCandles = seedLimit,
  alertLimit = DEFAULT_SIGNAL_ALERT_LIMIT,
  breakoutOptions = DEFAULT_BOX_RANGE_BREAKOUT_OPTIONS,
  ...
  const mergedBreakoutOptions = {
    ...DEFAULT_BOX_RANGE_BREAKOUT_OPTIONS,
    ...breakoutOptions,
  };
  const listeners = new Set();
  const emittedAlertIds = new Set();
  const minimumSeedCandles = mergedBreakoutOptions.rangeLookback + 1;

  if (seedLimit < minimumSeedCandles) {
    throw new Error(
      `seedLimit must be at least ${minimumSeedCandles} candles for box-range breakout detection`,
    );
  }

  if (maxCandles < minimumSeedCandles) {
    throw new Error(
      `maxCandles must be at least ${minimumSeedCandles} candles for box-range breakout detection`,
    );
  }
  ...
  const evaluateSignal = (symbol, interval, eventTime) => {
    ...
    const detection = detectBoxRangeBreakoutSignal(
      bucket.candles,
      mergedBreakoutOptions,
    );
